...
- **`make test` runs the harness in `test/`** with `--host
  wippy.terminal:host`; it writes `test/shots/antibug.png` (mid-scan) and
  `antibug-complete.png` (the box), the window as the shell's renderer drew it. Look at the picture: the geometry checks do not
